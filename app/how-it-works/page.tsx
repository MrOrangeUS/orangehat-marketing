...
const steps = [
  { title: "Audit", desc: "We assess your brand, outreach, follow-up, and current systems." },
  { title: "Build", desc: "We install your brand foundation, outreach engine, and automation backbone." },
  { title: "Deploy", desc: "Campaigns go live. Automation activates. Follow-up begins instantly." },
  { title: "Optimize", desc: "We refine messaging, routing, and workflows based on real engagement." },
  { title: "Scale", desc: "Once proven, add-ons like AI Recovery or PPL can be layered in." },
];
    ...
    <main className="text-white bg-[#050505] min-h-screen">
      ...
      <section className="py-24 px-6 bg-[#0f0f0f] border-t border-white/10">
        <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
          {steps.map((s, i) => (
            <div key={s.title} className="bg-[#151515] p-8 rounded-lg border border-white/10">
              <p className="text-xs uppercase tracking-widest text-white/60 mb-2">
                Step {i + 1}
              </p>
              <h2 className="text-2xl font-semibold mb-3 text-[--oh-orange]">{s.title}</h2>
              <p className="text-white/85">{s.desc}</p>
            </div>
          ))}
        </div>
